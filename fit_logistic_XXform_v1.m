function fit = fit_logistic_XXform_v1(sub, priorFlag)
...
for i = 1:length(L)
    ...
    dI = sub.dI;
    a = sub.choice;
    ind     = ([sub.gameLength] == L(i)) & (dI~=0) & (~isnan(a));
    c5      = a(ind)==2;
    
    dm = dR(ind);%sub.o2(ind,4)-sub.o1(ind,4);
    uc = dI(ind);%-[sub.n2(ind,4)-sub.n1(ind,4)]/2; % +1 if choice 2 is uncertain
    ...
    dm = dR(ind);%sub.o2(ind,4)-sub.o1(ind,4);
    uc = dI(ind);%[sub(1).n2(ind,4)-sub(1).n1(ind,4)]/2; % +1 if choice 2 is uncertain
