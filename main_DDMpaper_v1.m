...
for sn = 1:length(uu)
    ind = uu(sn) == (data{1});
    sim(sn).dR = data{3}(ind);
    sim(sn).dI = -data{4}(ind);
    sim(sn).RT = data{5}(ind);
    sim(sn).h = data{2}(ind);
    
    % DDM parameters x0	z	A	T0
    sim(sn).x0 = data{6}(ind);
    sim(sn).z = data{7}(ind);
    sim(sn).A = data{8}(ind);
    sim(sn).T0 = data{9}(ind);
...
for i1 = 1:length(sub)
    for i2 = 1:length(sim)
        ...
        hh(hh==5) = 1;
        hh(hh==10) = 6;
        isEq(i1,i2) = isequal(sim(i2).h, hh);
    end
end
...
for sn = 1:length(sim)
    ...
    sim(sn).T01 = unique(T0(i1));
    sim(sn).T06 = unique(T0(i6));
...
figure(1); clf;
set(gcf, 'Position', [811   572   500   250])
hg = [0.25 0.15];
wg = [0.37 0.1];
...
set(gca, 'ytick', [-z x0 z], ...
    'yticklabel', {'lower threshold, -z' 'starting point, x_0' 'upper threshold, +z'}, ...
...
text(x2*100, x0+40*A, ' signal-to-noise ratio,\mu', 'fontsize', 20)
...
priorFlag = 1;
for sn = 1:length(sub)
    fit(sn) = fit_biasNoiseBonus_v1(sub(sn), priorFlag)
end
...
text(1.5, 52, '*', 'fontsize', 30, 'horizontalalignment', 'center')
...
plot([1 1 2 2], [20 45 45 30], 'k-', 'linewidth', 1)
text(1.5, 46, '*', 'fontsize', 30, 'horizontalalignment', 'center')
...
plot([1 1 2 2], [15 45 45 40], 'k-', 'linewidth', 1)
text(1.5, 46, '*', 'fontsize', 30, 'horizontalalignment', 'center')
...
e1 = plot_choiceCurves_sim_v1(ax(1:2), sim, binEdges); %plot_rtCurves_sim_v1(ax(3:4), sim, 1)
...
e1 = plot_rtCurves_sub_v1(ax(1:2), sub, 0.1 , 10)
e2 = plot_rtCurves_sim_v1(ax(3:4), sim, 1)
set([e1([2 4]) e2([2 4])], 'visible', 'off')
...
e1 = plot_choiceCurves_v1(ax(1:2), sub, binEdges);; %plot_rtCurves_sub_v1(ax(1:2), sub)
...
e1 = plot_choiceCurves_v1(ax(1:2), sub, binEdges); % plot_rtCurves_sub_v1(ax(3:4), sub)
